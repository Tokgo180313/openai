...
export class ApiKeyService {
    constructor(
        // 注入ApiKey模型
        private readonly apiKeyModel: ApiKeyDocument,
      ) {}
    //添加
    async createApiKey(apiKeyDto: ApiKeyEntity ): Promise<ApiKeyEntity> {       
        // 实现创建API Key的逻辑
        const createApiKey = new this.apiKeyModel(apiKeyDto);
        return await createApiKey.save();},

    //查询
    async findApiKeyList(apiKeyDto:ApiKeyDto): Promise<ApiKeyEntity[]> {
        // 实现查询API Key列表的逻辑
